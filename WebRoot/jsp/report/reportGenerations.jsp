...
<html>
  ...
  <body>
	<form action="download.html" method="post" name="batchForm" id="batchForm">
		<div class="place">
	    <span>位置：</span>
	    <ul class="placeul">
	        ...
	        <c:choose>
				<c:when test="${not empty batchList}">
			        ...
			        <c:choose>
					 ...
					 <c:choose>
					       <c:when test="${batch.downloadStatus==0}">
					             <td><font color=red>未下载</font></td>
					       </c:when>
					       <c:when test="${batch.downloadStatus==1}">
					             <td><font color=red>部分下载</font></td>
					       </c:when>
					       <c:otherwise>
					              <td><font color="#009900">全部下载</font></td>
					       </c:otherwise>
					</c:choose>
					<td>
			        	<a href="#" class="tablelink" onclick="window.location.href='${ctx}/reportAudit/reportShow.html?batchId=${batch.batchId}'">详情</a> &nbsp;|&nbsp;
			        	<a href="#" class="tablelink" onclick="generateBatch(${batch.batchId})">整单下载</a> 
			        </td>
					</tr>
					</c:forEach>
				</c:when>
	 ...
	 <script type="text/javascript">
		...
		function search(){
			$("#batchForm").attr("action", "download.html");
			$("#batchForm").submit();
		}
		//报告生成与下载-整批下载
		function generateBatch(id){
			if(id == null) {
				...
				window.location.href="${ctx}/reportAudit/generateBatch.html?batchId="+id;
			}
		}
		</script>
	</body>
